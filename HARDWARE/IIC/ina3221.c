...
#include <stdlib.h>
/*****************硬件连接定义*********************/
#define INA3221_SCL_PORT GPIOG
#define INA3221_SCL_PIN GPIO_Pin_12
#define INA219_I2C_GPIO_CLOCK            RCC_APB2Periph_GPIOG
#define INA3221_SDA_PORT GPIOG
#define INA3221_SDA_PIN GPIO_Pin_11

/************************延时接口****************************/
#define delay_nms_3221 delay_ms       //毫秒延时
...
#define IIC_SCL_L_3221 GPIO_ResetBits(INA3221_SCL_PORT, INA3221_SCL_PIN)
#define IIC_SCL_H_3221 GPIO_SetBits(INA3221_SCL_PORT, INA3221_SCL_PIN)

#define IIC_SDA_L_3221 GPIO_ResetBits(INA3221_SDA_PORT, INA3221_SDA_PIN)
#define IIC_SDA_H_3221 GPIO_SetBits(INA3221_SDA_PORT, INA3221_SDA_PIN)
#define SDA_READ_3221 GPIO_ReadInputDataBit(INA3221_SDA_PORT, INA3221_SDA_PIN)
...
void SCL_OUT_3221(void)
{
    ...
    GPIO_InitStructure.GPIO_Pin = INA3221_SCL_PIN;
    GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
    GPIO_InitStructure.GPIO_Mode = GPIO_Mode_Out_PP;
    GPIO_Init(INA3221_SCL_PORT, &GPIO_InitStructure);
    GPIO_SetBits(INA3221_SCL_PORT, INA3221_SCL_PIN);
	 
}
void SDA_OUT_3221(void)
{
    GPIO_InitTypeDef  GPIO_InitStructure;
    ...
    GPIO_InitStructure.GPIO_Pin = INA3221_SDA_PIN;
    GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
    GPIO_InitStructure.GPIO_Mode = GPIO_Mode_Out_PP;
    GPIO_Init(INA3221_SDA_PORT, &GPIO_InitStructure);
    GPIO_SetBits(INA3221_SDA_PORT, INA3221_SDA_PIN);
	    

}
 
void SDA_IN_3221(void)
{
    GPIO_InitTypeDef  GPIO_InitStructure;
...
void INA3221_Init(uint8_t addr)
{
    ...
    INA3221_SendData(addr, INA3221_CFG_REG, 0x7127); // 7127为默认配置
	   //INA3221_SendData(addr, INA3221_CFG_REG, 0x74E7); //采样时间8ms,运作良好，但响应速度慢
	  //INA3221_SendData(addr, INA3221_CFG_REG, 0x72E7);//最终选择
...
int INA3221_Mean_filtering1(void){
	const int count=5; //均值滤波的数据量
	int i=0;
	int sum=0;
	for(i=0;i<count;i++)
		sum+=INA3221_GetShuntVoltage(0x80,1);
	return sum/count;
}
//对获取的数据进行中值滤波
int INA3221_Median_filtering1(void)
{	int i;
	s32 a;
  ...
  static s32 nums[count]; //可能需要对数组初始化,debug看一下初始值是否为0;
	...
	for(i=0;i<count-1;i++)
		nums[i]=nums[i+1];
	nums[count-1]=INA3221_GetShuntVoltage(0x80,1);
	
	for(i=0;i<count;i++)
		nums1[i]=nums[i];
	 qsort(nums1, count, sizeof(nums1[0]), compInc); //排序
	...
	*/
	...
	s32 nums1[count];
	for(i=0;i<count;i++)
		nums[i]=INA3221_GetShuntVoltage(0x80,1);
	for(i=0;i<count;i++)
		nums1[i]=nums[i];
	 qsort(nums1, count, sizeof(nums1[0]), compInc); //排序
	if(count%2==0) {          //奇偶取中位数的方法不一样
...
int INA3221_Median_filtering2(void)
{	int i;
	s32 a;
	const int count=5; //中值滤波的数据量
	/*
  static s32 nums[count]; //可能需要对数组初始化,debug看一下初始值是否为0;
	...
	for(i=0;i<count-1;i++)
		nums[i]=nums[i+1];
	nums[count-1]=INA3221_GetShuntVoltage(0x80,2);
	...
	*/
	...
	s32 nums1[count];
	for(i=0;i<count;i++)
		nums[i]=INA3221_GetShuntVoltage(0x80,2);
	for(i=0;i<count;i++)
		nums1[i]=nums[i];
	 qsort(nums1, count, sizeof(nums1[0]), compInc); //排序
	if(count%2==0) {          //奇偶取中位数的方法不一样
...
int INA3221_Median_filtering3(void)
{	int i;
	s32 a;
	const int count=5; //中值滤波的数据量
	/*
  static s32 nums[count]; //可能需要对数组初始化,debug看一下初始值是否为0;
	...
	for(i=0;i<count-1;i++)
		nums[i]=nums[i+1];
	nums[count-1]=INA3221_GetShuntVoltage(0x80,3);
	...
	*/
	...
	s32 nums1[count];
	for(i=0;i<count;i++)
		nums[i]=INA3221_GetShuntVoltage(0x80,3);
	for(i=0;i<count;i++)
		nums1[i]=nums[i];
	 qsort(nums1, count, sizeof(nums1[0]), compInc); //排序
	if(count%2==0) {          //奇偶取中位数的方法不一样
